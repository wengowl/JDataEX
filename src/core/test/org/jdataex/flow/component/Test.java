...
import org.jdataex.flow.component.step.dataaccess.ComplexAutoDataAccess;
import org.jdataex.flow.component.step.dataaccess.ReDataAccess;
import org.jdataex.flow.component.step.trigger.TriggerComponent;
...
//         System.out.println(i);
        ...
        TriggerComponent triggerComponent = new TriggerComponent(10,10,"oracle","pool");
//       triggerComponent.configHandler();
...
//        ReDataAccess reDataAccess = new ReDataAccess();
       /* DBManager dbManager = DBManager.getInstance();
        ...
        /*String sql = "insert into JJJC_AJSLQK(ID,APP_CODE,DEPT_CODE,REG_TIME,UPDATE_TIME,AJBS)values(?,?,?,?,?,?)";
        try {
            PreparedStatement preparedStatement= basePoolConnection.prepareStatement(sql);
            preparedStatement.setObject(1, UUID.randomUUID().toString());
            preparedStatement.setObject(2, "GF2009－01");
            preparedStatement.setObject(3, "GF2009－01");
            preparedStatement.setObject(4, "10:20:33", Types.TIMESTAMP);
            preparedStatement.setObject(5, new Timestamp(System.currentTimeMillis()));
            preparedStatement.setObject(6, 2428342);
            ...
            e.printStackTrace();
      /*  }*//*
        String sql ="select BZ from JJJC_BSCRXX";
        try {
            ResultSet set = basePoolConnection.executeQuery(sql);
            while (set.next()){
                System.out.println(set.getBlob(1));
                ByteArrayInputStream msgContent =(ByteArrayInputStream) set.getBlob(1).getBinaryStream();
