...
public class RowDataAccessProcess {
     ...
     *单条数据插入
     */
    public int insertProcess(Connection connection,IRow row) throws SQLException {
...
//                   // 首先转换编码为UTF-8，然后再BASE64化,存为blob
     ...
     *单条数据删除
     */
    public int deleteProcess(Connection connection,IRow row) throws SQLException {
        int size = row.getColumnsSize();
        if (size == 0) {
            LoggerFactory.getRootLogger().info("没有字段记录！");
            return 1;
...
//        去除blob字段作为条件
     ...
     *单条数据更新
     */
    public int updateProcess(Connection connection,IRow row) throws SQLException {
       /* IColumn column = row.getColumn("REG_TIME") ;
...
//       System.out.println("freeMemory（）"+Runtime.getRuntime().freeMemory() );
        ...
        if (size<=4&& row.getColumn(0).getName().equalsIgnoreCase("AJBS")){
         ...
         int a=0;
        boolean flag =false;
        List<String> conditons_column = ReadDataConfig.getInstance().getConditionColumnByTable(row.getHeader().get("tableName"));
        if (conditons_column!=null){
          //  a=conditons_column.size();
            flag=true;
        }
        String[] names = new String[size];
        Object[] values = new Object[size];
        ...
        for (int i = 0; i < size; i++) {
            names[i] = row.getColumn(i).getName();
            values[i] = row.getColumn(i).getValue();
//            System.out.println("值："+values[i]+"  "+ CommonUtil.isNullValue(values[i]));
            types[i] = columnTypetoSQLType(row.getColumn(i).getType());
            if (types[i]!=Types.BLOB&&CommonUtil.isNotNullValue(values[i])){
                a=a+1;
            }
