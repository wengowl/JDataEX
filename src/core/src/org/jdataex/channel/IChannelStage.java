...
public interface IChannelStage<T>{
	 ...
	 * @return
	 */
	public abstract IChannelStage<T> addBefore(IHandler<T> handler); 
...
//|||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||	
	...
	/**
	 * 特殊业务用于触发本地stage channelInput 的方法.本方法主要适用于自启动流程,执行定时
	 * 任务等等.
	 * 注意:只有localChannelInput或者nioSocketChannel才接受此方法.
	 * @param msg
	 * @return
	 */
	 ...
	 * 通过channelStage完成.
	 * @param IRowSet  
	 * @param props 形式 file=ddddd;xxx=ddd;sss=ddd;
	 * @param channelOutputName 如果有多个接收端(channelOutput),可以指定需要写入的
	 * 接收端,如果只有一个接收端,可以写null
	 * @return {@link IChannelFuture}
	 */
	public abstract IChannelFuture<T>[] writeMsg(T msg, String props, String[] channelOutputName);
