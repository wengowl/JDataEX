 ...
 * 数据入库操作处理
 */
public class ComplexDataAccessProcess extends BaseComponent {
    ...
    public int updateProcess(IRow row) throws SQLException {
        ...
        if (referencekey != null) {
            ...
            values[size] = row.getColumn(size).getValue(); //最后一个值为参考条件的字段值
            types[size] = columnTypetoSQLType(row.getColumn(referencekey).getType());
        } else {       //默认第一个字段为参考条件的字段值
            names[size] = names[0];
...
//        }
    ...
    public int insertMsgCacheProcess(IRow row, String failedtxt, String operationtype) {
        ...
        types[6] = Types.LONGVARCHAR;                        //对应TEXT
        String sql = "INSERT INTO MsgCacheTable"
                ...
                + "VALUES(?, ?, ?, ?, ?, ?, ?)";
        String database = null;    //插入到默认库，即第一个数据库
        if (row.getHeader().containsKey("database")) {
           // database = row.getHeader().get("database");
        }
        try {

            i = dbOperation.sqlUpdate(sql, objects, types, database);
