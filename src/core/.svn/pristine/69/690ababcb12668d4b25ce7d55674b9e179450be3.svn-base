...
import org.jdataex.channel.mock.MockTask;
...
public class DefaultExecutorTest extends EasyMockSupport {
	...
	/**
	 * 测试shutdown后再执行exec的情况
	 */
	@Test
	public void testShutdownToExe(){
		exer.shutdown();
		try{
			IChannelFuture<String> ifs = exer.execute(createTask());
			ifs.isSuccess();
		}catch(Exception e){
			System.out.println("关闭后就应该错误");
